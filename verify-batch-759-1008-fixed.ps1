...
foreach($p in $pairs){foreach($source in @('/tools/'+$p[0],'/tools/'+$p[0]+'/')){$dest='/tools/'+$p[1]+'/';$pattern='(?m)^\s*'+[regex]::Escape($source)+'\s+'+[regex]::Escape($dest)+'\s+301\s*$';if(-not [regex]::IsMatch($red,$pattern)){throw "Missing redirect: $source -> $dest 301"}}}
