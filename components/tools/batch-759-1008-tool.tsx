...
function base58Encode(bytes:Uint8Array){ let x=0n; for(const b of bytes)x=(x<<8n)+BigInt(b); let s=''; while(x>0){s=B58[Number(x%58n)]+s;x/=58n} for(const b of bytes){if(b===0)s='1'+s;else break} return s||'1'; }
function base58Decode(s:string){ let x=0n; for(const c of s.trim()){const i=B58.indexOf(c);if(i<0)throw new Error('Invalid Base58 input.');x=x*58n+BigInt(i)} const arr:number[]=[]; while(x>0){arr.unshift(Number(x&255n));x>>=8n} for(const c of s){if(c==='1')arr.unshift(0);else break} return new Uint8Array(arr); }
...
function calculate(kind:Batch7591008Kind,v:Record<string,string>):ToolResult {
  switch(kind) {
    ...
    case "unix-nanoseconds-converter": {
      const ns=BigInt(txt(v,'ns').trim()); const ms=Number(ns/1000000n); return out(new Date(ms).toISOString());
    }
