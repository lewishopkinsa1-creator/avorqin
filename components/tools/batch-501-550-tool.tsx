...
import {
  BATCH501550_CONFIG,
  runBatch501550,
  type Batch501550Kind,
  type Batch501550ResultItem,
} from "@/lib/tool-utils/batch-501-550-electrical";
...
function ResultGrid({ items }: { items: Batch501550ResultItem[] }) {
  return (
...
export function Batch501550Tool({ kind }: { kind: Batch501550Kind }) {
  const config = BATCH501550_CONFIG[kind];
  ...
  const [summary, setSummary] = useState<Batch501550ResultItem[]>([]);
  ...
  const run = () => {
    ...
    try {
      const result = runBatch501550(kind, values);
