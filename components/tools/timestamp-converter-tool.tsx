...
import { timestampToDate, dateToTimestamp, getCurrentTimestamp } from "@/lib/tool-utils/timestamp-utils";
import { Clock, ArrowRightLeft } from "lucide-react";

export function TimestampConverterTool() {
  const [input, setInput] = useState('');
  const [unit, setUnit] = useState<'seconds' | 'milliseconds'>('seconds');
  const [direction, setDirection] = useState<'ts-to-date' | 'date-to-ts'>('ts-to-date');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted rounded-md px-3 py-2">
        ...
        <span>Current Unix timestamp: <strong>{liveTs}</strong> seconds</span>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <Select
          label="Direction"
          value={direction}
          onChange={(e) => setDirection(e.target.value as 'ts-to-date' | 'date-to-ts')}
          className="sm:w-56"
        >
      ...
      <div className="flex gap-2">
        <Input
          placeholder={direction === 'ts-to-date' ? 'Enter timestamp...' : 'Enter date (e.g. 2024-01-15T10:30:00Z)...'}
          value={input}
          onChange={(e) => setInput(e.target.value)}
          aria-label={direction === 'ts-to-date' ? 'Timestamp input' : 'Date input'}
          className="font-mono text-sm flex-1"
        />
        <Button variant="outline" onClick={handleUseCurrent} title="Use current timestamp">
          <Clock className="h-4 w-4" />
        </Button>
      </div>
