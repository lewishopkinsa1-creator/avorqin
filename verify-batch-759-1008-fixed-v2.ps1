...
foreach($p in $pairs){$sourceNoSlash='/tools/'+$p[0];$sourceSlash=$sourceNoSlash+'/';$dest='/tools/'+$p[1]+'/';foreach($source in @($sourceNoSlash,$sourceSlash)){$pattern='(?m)^\s*'+[regex]::Escape($source)+'\s+'+[regex]::Escape($dest)+'\s+301\s*$';if(-not [regex]::IsMatch($red,$pattern)){throw "Missing redirect: $source -> $dest 301"}}}
